...
The primary challenge addressed in this solution is the **severe class imbalance** in the training data, where high scores (9.0–10.0) dominate, and low scores (0.0–6.0) are extremely rare.
...
### Data Cleaning
Rare fractional scores were consolidated to standardize the target variable (e.g., mapping 9.5 → 10.0, 4.0 → 3.0).
...
*   **Architecture:**
    *   **Layer 1:** Cross-Attention (Prompt ↔ Response) to generate a context-aware prompt.
    *   **Layer 2:** Cross-Attention (Metric ↔ Context-Aware Prompt) to align with the evaluation criteria.
    *   **Head:** Regression MLP.
*   **Optimization (Optuna):** Used to tune hyperparameters (LR, Dropout, Heads) and **custom loss weights**.
*   **Loss Function:** Weighted MSE. Weights were dynamically tuned to penalize errors on rare classes (scores < 6.0) significantly more (Weight ≈ 1000x) than common classes.
